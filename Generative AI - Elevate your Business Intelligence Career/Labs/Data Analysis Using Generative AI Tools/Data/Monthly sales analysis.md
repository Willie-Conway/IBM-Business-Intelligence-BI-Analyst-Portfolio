...
We generate a list of 100 customer transactions. Each transaction includes `customer_id`, `transaction_id`, `transaction_date`, `transaction_amount`, `product_category`, and `payment_method`.

---

## Sample Dataset (First 30 Transactions)
...
* The dataset reflects a diverse range of transaction amounts across categories.
...
### Example Calculation (Customers 1–30)
...
* Balanced distribution in "Low" and "Medium" spenders.
* Smaller percentage are high spenders.

---

## Exercise 2: Analyzing Payment Methods

**Prompt:** Analyze the distribution of payment methods.
...
* Distribution helps optimize payment processing and promotions.

---

## Exercise 3: Visualizing Transaction Trends

**Prompt:** Create a line chart showing the trend of transactions over the year.
...
* Flat trend line; consistent number of transactions in Jan–Feb.
...
* Mode of category used for multiple transactions.
* Ties can be resolved by earliest or arbitrary selection.
...
* Reveals spending patterns, low/medium/high transactions.
* Identifies outliers for business strategy.
...
* High-value customers are crucial for loyalty programs.
* Top spenders contribute significantly to sales.

---

## Exercise 7: Correlation Analysis

**Prompt:** Analyze the correlation between transaction amount and product category.

* Convert `product_category` into numeric codes for correlation analysis (e.g., Electronics=1, Clothing=2, Grocery=3).
* Compute correlation using Pearson or Spearman method.
...
df = pd.DataFrame({
    ...
    'product_category': ['Electronics','Clothing','Grocery']*33 + ['Electronics']
})
df['product_category_code'] = df['product_category'].map({'Electronics':1, 'Clothing':2, 'Grocery':3})

correlation = df['transaction_amount'].corr(df['product_category_code'])
print(f'Correlation between amount and category: {correlation:.2f}')
```
...
**Highest Sales Month:** January 2023 ($1,500.00)

**Insights:**

* Helps plan inventory, promotions, and forecasting.
